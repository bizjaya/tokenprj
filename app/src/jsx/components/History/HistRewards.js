...
const HisRewardsTable = () => {
  ...
  const data = useSelector((state) => state.referral.HistRwds);
  const paging = useSelector((state) => state.referral.paging);

  const dispatch = useDispatch();
  const { getHistRwds } = bindActionCreators(referralActions, dispatch);

  const [CurPage, setCurPage] = useState(1);

  useEffect(async () => {
    if (window.notempty(wallet.Acct)) {
      getHistRwds({ Id: wallet.Acct });
    }
  }, [wallet.Acct, pathname]);

  const TurnPage = () => {
    getHistRwds({ Id: wallet.Acct, ...paging, PageNo: CurPage });
  };
...
export default HisRewardsTable;
